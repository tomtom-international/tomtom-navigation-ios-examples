...
///
/// You can experiment with different ways of showing the current location on the map
/// by changing the locationIndicatorType in the onMapReady callback.
...
/// The virtual map is observed through a camera that can be
/// zoomed, panned, rotated, and tilted to provide a compelling 3D navigation experience.
...
/// Add an extension to NavigationController with a navigateToCoordinate function that plans a route,
/// starts location simulation and starts the navigation process.
///
/// The call to the async planRoute function is wrapped in a Task
/// so that it can be called from the navigateToCoordinate function, even though that function is not async.
...
/// Update the MapCoordinator extension with the observe function
/// to display changes to the current route and its progress on the map.
///
/// After adding a route on the map, you will receive a reference to that route.
/// The addRouteToMap function keeps it in the routeOnMap so that it can show the visual progress along the route,
/// without being added again.
/// The default location provider has an approximate position but no route information.
/// For smooth movement of the chevron along the route,
/// ensure the map uses a mapMatchedLocationProvider from Navigation instead of the SimulatedLocationProvider.
extension MapCoordinator {
...
/// Update the NavigationController extension with the onNavigationViewAction function
/// to handle actions like arrival, mute, and etc
extension NavigationController {
